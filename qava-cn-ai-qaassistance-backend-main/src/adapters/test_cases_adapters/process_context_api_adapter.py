...
class ProcessContextAdapterApi(ProcessContextPortApi):
    def __init__(self, llm: LLMPort, mongo_repo: MongoDBPort, name_history_generator: NameHistoryGeneratorPort, conversation_manager: ConversationManagementPort, count_tokens: CountTokensPort):
        self.llm = llm
...
datetime.fromtimestamp(timestamp, timezone.utc) # Cumple
        ...
        validation_message = {
            ...
            "content": (
                ...
                if is_valid_context
                    ...
                    "Parece que el contexto proporcionado no es válido para generar casos de prueba de API. "
                    "Por favor, proporciona un contexto relacionado con la documentación de la API, como Swagger, GraphQL o similares. "
                    "Esto nos permitirá generar casos de prueba más precisos y relevantes para tu API."
                )
            ),
    ...
    def validate_context(self, context: str) -> bool:
        ...
        # Asegúrate de que response_tuple sea un tipo esperado
        ...
        if response_normalized == 'true':
            return True
        elif response_normalized == 'false':
            return False
        else:
            self.logger.warning(f"Respuesta inesperada del LLM: {response_normalized}")
            return False

    def prompt_validation_context(self, context: str) -> str:
        return f"""
        Solo responde con 'true' o 'false'.
        
        ¿El siguiente texto es un contexto es de codigo de un swagger, graphql o openapi ?
